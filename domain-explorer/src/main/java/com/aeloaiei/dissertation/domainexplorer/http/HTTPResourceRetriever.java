    ...
    public Optional<RawWebResource> retrieve(UniformResourceLocator url, String userAgent) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                rawWebResource = Optional.of(new RawWebResource(url, "", HttpStatus.resolve(response.code())));
                ...
                rawWebResource = Optional.of(new RawWebResource(url, response.body().string(), HttpStatus.resolve(response.code())));
            }
        } catch (IOException e) {
